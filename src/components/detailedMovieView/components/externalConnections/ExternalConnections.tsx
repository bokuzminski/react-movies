import { Facebook, Instagram, LinkOutlined, X } from "@mui/icons-material";
...
export const ExternalConnections = ({ homepage, data }: ExternalConnectionsProps) => {
  ...
  return (
    <Stack direction={"row"} gap={1}>
      {homepage && (
        <Button variant="outlined" startIcon={<LinkOutlined />} onClick={openHomePage}>
          Homepage
        </Button>
      )}

      {twitter_id && (
        <Button variant="outlined" startIcon={<X />} onClick={openTwitter}>
          Twitter
        </Button>
      )}

      {facebook_id && (
        <Button variant="outlined" startIcon={<Facebook />} onClick={openFacebook}>
          Facebook
        </Button>
      )}

      {instagram_id && (
        <Button variant="outlined" startIcon={<Instagram />} onClick={openInstagram}>
          Instagram
        </Button>
      )}
  ...
  function openFacebook() {
    window.open(`https://facebook.com/${facebook_id}`, "_blank");
  }
